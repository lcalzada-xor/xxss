...
func TestGeneratePayload(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			context:  models.ContextHTML,
			allowed:  []string{"a", "b"}, // Restrictive allowed list to force fallback
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			got := GenerateReflectedPayload(tc.context, tc.allowed, nil)
			if got != tc.expected {
				t.Errorf("Expected payload '%s', got '%s'", tc.expected, got)
			}
		})
	}
}
